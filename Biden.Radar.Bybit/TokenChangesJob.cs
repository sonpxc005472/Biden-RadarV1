...
public class TokenChangesJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        if (currentSpotSymbols.Count != SharedObjects.SpotSymbols.Count)
        {
            ...
            {
                var newMarginTokensAdded = currentSpotSymbols.Where(x=> x.MarginTrading == MarginTrading.Both || x.MarginTrading == MarginTrading.UtaOnly).Select(x => x.Name).Except(SharedObjects.SpotSymbols.Where(x=> x.MarginTrading == MarginTrading.Both || x.MarginTrading == MarginTrading.UtaOnly).Select(s => s.Name)).ToList();
